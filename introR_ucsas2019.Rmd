...
## Getting Started
- The workshop slides and codes are available at the [link](https://github.com/mdtuhinsheikh/R_Intro_UCSAS2019).
- Prerequisites: 
...
- **Quiz:** Let $A = \begin{bmatrix} 1 & 2 \\ 3 & 4  \end{bmatrix}$ and $B = \begin{bmatrix} 10 & 20 \\ 30 & 40  \end{bmatrix}$. 
  ...
  + Is $AB$ possible, print using R program using logical operator?
...
if (dim(A)[2] == dim(B)[1]){
  print("The multiplication possible")
} else {
...
## Function cont...
- **Quiz:** Write a function that takes $x$ and $y$ as input and evaluates $x^y$.
```{r echo = TRUE, eval = FALSE}
pow <- function(x, y) {
  result <- x^y
  return(result)
...
#[1] 1 2 3 4 5
```
- We can write any type of operation in between the loop which needs to be repeated.
